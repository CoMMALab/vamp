...
namespace vamp::planning::constraint
{
    ...
    class ClosedLinkConstraint : public RobotConstraint<Robot, rake>
    {
        protected:
            ...
            {
                vamp::FloatVector<rake, Robot::num_closed_chains * Robot::dimension> J;  // jacobian
                vamp::FloatVector<rake, Robot::num_closed_chains> err;                   // error vector

                auto &operator[](size_t index)
                {
                    if (index < Robot::num_closed_chains * Robot::dimension)
                    {
                        return J[index];
                    }
                    else if (
                        index >= Robot::num_closed_chains *  Robot::dimension &&
                        index < Robot::num_closed_chains * Robot::dimension + Robot::num_closed_chains)
                    {
                        return err[index - Robot::num_closed_chains * Robot::dimension];
                    }
                    else
                    {
                        return err[0];
                ...
                JacobianProjectInp &
                operator=(vamp::FloatVector<rake, Robot::num_closed_chains + Robot::num_closed_chains * Robot::dimension> y)
                {
                    for (size_t i = 0; i < Robot::num_closed_chains; i++)
                    {
                        err[i] = y[Robot::num_closed_chains * Robot::dimension + i];
                    }
                    for (size_t i = 0; i < Robot::num_closed_chains * Robot::dimension; i++)
                    {
                        J[i] = y[i];
                    }
                    return *this;
        ...
        public:
            ...
            vamp::FloatVector<rake, 1> print_robot_tsr_error(const ConfigurationBlock &q) const
            {
                ...
                for(auto i=0U; i < Robot::num_closed_chains * Robot::dimension; i++){
                    if (i%Robot::dimension == 0)
                        std::cout << std::endl << " J[" << i << "]: ";
                    std::cout << std::setprecision(5) << jac_proj_inp.J[{i, 0}] << " ";
                }
                std::cout << std::endl << "Error : ";
                for(auto i=0U; i < Robot::num_closed_chains; i++)
            ...
            vamp::FloatVector<rake, 1> distanceToConstraint(const ConfigurationBlock &q) const
            {
                ...
                for (size_t i = 1; i < Robot::num_closed_chains; i++)
                {
                ...
                float alpha = 1.0F)
            {
                ...
                RobotConstraint<Robot, rake>::integrateJointConfiguration(q, q, grad, alpha);
            }
