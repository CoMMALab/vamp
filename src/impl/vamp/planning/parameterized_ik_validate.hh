...
namespace vamp::planning
{
        ...
        const collision::Environment<FloatVector<rake>> &environment) -> bool
    {
        ...
        FloatVector<rake, Robot::dimension + 4> block;
        ...
        auto [param_valid, full_ambient_configuration] = Robot::template parameterized_ik<FloatVector<rake, Robot::dimension + 4>, rake>(block);
        if (not param_valid)
        {
            return false;
        }
        ...
        bool valid = (environment.attachments) ? Robot::template fkcc_attach<rake>(environment, full_ambient_configuration) :
                                                 Robot::template fkcc<rake>(environment, full_ambient_configuration);
        if (not valid or n == 1)
        {
            return valid;
        }
