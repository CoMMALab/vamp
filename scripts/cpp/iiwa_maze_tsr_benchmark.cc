...
//
// Usage:
//   vamp_iiwa_maze_tsr_benchmark [paths_json] [results_csv]
//
...
auto main(int argc, char **argv) -> int
{
    ...
    const std::filesystem::path results_csv_path =
        (argc > 2) ? argv[2] : "results/iiwa_maze_tsr_benchmark.csv";
    ...
    for (const auto &problem : problems)
    {
        ...
        if (result.path.size() > 0)
        {
            ...
            results_csv << "iiwa_maze_tsr," << (total_num_problems - 1) << ",,1," << (result.nanoseconds / 1.0e6)
        ...
        else
        {
            ...
            results_csv << "iiwa_maze_tsr," << (total_num_problems - 1) << ",,0," << (result.nanoseconds / 1.0e6)
                        << "," << result.iterations << ",,,\n";
        }
