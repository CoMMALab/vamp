...
namespace vamp::robots
{
    struct PandaTOPPLE
    {
        ...
        inline static std::pair<std::vector<npy_matrix>, std::vector<npy_matrix>> load_matrices() noexcept
        {
            // get numpy matrix path
            std::string file_path = __FILE__;
            std::filesystem::path npy_path(file_path);
            npy_path = npy_path.parent_path() / "numpy" / "panda";

            // load numpy matrices as eigen
            std::vector<std::string> files;
            for (const auto& file : std::filesystem::directory_iterator(npy_path)) {
                // Check if it's a regular file (skips folders/symlinks)
            ...
            std::vector<npy_matrix> weights;
            std::vector<npy_matrix> bias;
