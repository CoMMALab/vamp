...
def replace_package_path(urdf_path):
    ...
    replace_with = os.path.join(os.getcwd(), *parents)
    ...
    modified_content = re.sub(r'meshes/', "/src/resources/digit_description/meshes/", urdf_content)
    # modified_content = re.sub(r'package://urdf', replace_with, urdf_content)
...
def init_viz():
    ...
    _model, coll_model, vis_model = pinocchio.buildModelsFromUrdf(
        replace_package_path('resources/digit_description/digit_model_trace_collision_spherized.urdf'), '/src/vamp/resources/digit_description/meshes',)
