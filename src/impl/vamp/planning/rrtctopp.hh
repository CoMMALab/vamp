...
namespace vamp::planning
{
    ...
    struct RRTCTOPP
    {
        ...
        // modified from rrtc to account for bezier collision checking
        // may need to devise a different distance metric based on time
        inline static auto solve(
            const Configuration &start,
            ...
            typename RNG::Ptr rng) noexcept -> PlanningResult<Robot>
        {
            ...
            // check for a direct path to a goal
            for (const auto &goal : goals)
            {
                // need to modify validate_motion to check along bezier curve
                if (validate_bez_motion<Robot, rake, resolution>(start, goal, environment))
                {
