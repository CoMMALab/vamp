...
from ._core import TOPPLESettings as TOPPLESettings
from ._core import AOTOPPLESettings as AOTOPPLESettings
...
def configure_robot_and_planner_with_kwargs(robot_name: str, planner_name: str, **kwargs):
    ...
    if planner_name == "rrtc":
        ...
        if robot_name in ROBOT_RRT_RANGES:
            plan_settings.rrtc.range = ROBOT_RRT_RANGES[robot_name]
    elif planner_name == "topple":
        plan_settings = TOPPLESettings()
        if robot_name in ROBOT_RRT_RANGES:
            plan_settings.rrtc.range = ROBOT_RRT_RANGES[robot_name]
    elif planner_name == "aotopple":
        plan_settings = AOTOPPLESettings()
        if robot_name in ROBOT_RRT_RANGES:
            plan_settings.rrtc.range = ROBOT_RRT_RANGES[robot_name]
    ...
    if planner_name == "aorrtc" or planner_name == "topple" or planner_name == "aotopple":
