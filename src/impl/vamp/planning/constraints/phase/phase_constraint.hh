...
{
    // Type-erased phase-space inequality gate g(q, qd) <= 0 over a rake-wide block of flat
    // states z = (q, qd). Unlike Constraint, a phase gate defines no manifold: states are
    // checked against it, never projected onto it, and feasibility is restored by scaling
    ...
    struct PhaseConstraint
    {
        ...
        // Whether one SIMD lane of the last evaluate() block satisfies the gate.
        virtual auto extract(std::size_t lane) const noexcept -> bool = 0;

        // Largest s in (0, 1] such that (q, s * qd) satisfies the gate; exactly 1 if
